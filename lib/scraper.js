...
/**
 * The function collects all the promises and handle them together.
...
module.exports = function (link) {
    ...
    Promise.all([promise1, promise2, promise3]).then(function(values){ // fetching all the three promises

        let movies = [];  //all movies
        let notFullyBooked = [];

        values[0].forEach(function(weekday) {  //pushing and finding movies not fully booked into an array
        ...
        movies.forEach(function(movie){ //checking movies status(fully booked or not)
         if(movie.status){
        ...
        notFullyBooked.forEach(function(movie) {    //checking dinner and movies,
                                                    // movies needs to be 2h before the dinner starts.
        ...
        return validatedMovies;
            ...
            resolve(returnArrayWithResult(validatedMovies));  //returns the final results to the middleware
